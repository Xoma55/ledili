       ...
       <table class="be-list">
        <tfoot>
         <tr>
          ...
          <?php } else if ($field == 'product_stickers') { ?>
          <td class="left" width="1%">
           <div class="be-scrollbox" style="height:140px;">
            <?php foreach ($product_stickers as $product_sticker) { ?>
            <div><label><input name="product_<?php echo $field; ?>[]" type="checkbox" value="<?php echo $product_sticker['product_sticker_id']; ?>" /> <?php echo $product_sticker['name']; ?></label></div>
            <?php } ?>
           </div>
          </td>
          <?php } else { ?>
	...
	xhr = $.ajax({type:'POST', dataType:'json', data:data, url:'index.php?route=batch_editor/data/getProductData&token=' + token,
		success: function(json) {
			$.each(json, function(product_id, value) {
				...
				} else if (field == 'product_stickers') {
					var box = $('#product .be-scrollbox[data-product_id=\'' + product_id + '\']');
					
					box.find('input[type=\'checkbox\']').prop('checked', false);
					
					$.each(value, function(index, product_sticker_id) {
						box.find('input[type=\'checkbox\'][value=\'' + product_sticker_id + '\']').prop('checked', true);
					});
...
$(document).ready(function() {
	...
	$('#product_box').delegate('#product tbody .be-scrollbox input', 'click', function(e) {
		var box = $(this).parents('div.be-scrollbox');
		var product_id = box.attr('data-product_id');
		var value = '';
		
		box.find('input[type=\'checkbox\']:checked').each(function(index, element) {
			value += '&product_product_stickers[]=' + $(element).val();
		});
		
		xhr = $.ajax({type:'POST', dataType:'json', data:'selected[]=' + product_id + '&field=product_stickers' + value, url:'index.php?route=batch_editor/index/editProduct&token=' + token,
			error: function() {
