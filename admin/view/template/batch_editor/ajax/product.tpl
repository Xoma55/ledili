...
<table id="product" class="be-list">
 ...
 <tbody>
  ...
  <tr class="selected_row-<?php echo $product['product_id']; ?> <?php echo $class; ?>">
   ...
   <td class="left">
    <div class="be-scrollbox" data-product_id="<?php echo $product['product_id']; ?>" style="height:140px;">
     <?php foreach ($product_stickers as $product_sticker) { ?>
     <div>
      <label>
       <?php if (in_array ($product_sticker['product_sticker_id'], $product[$field])) { ?>
       <input type="checkbox" value="<?php echo $product_sticker['product_sticker_id']; ?>" checked="checked" />
       <?php } else { ?>
       <input type="checkbox" value="<?php echo $product_sticker['product_sticker_id']; ?>" />
       <?php } ?>
...
$('#product .filter input[name=\'table[<?php echo $table; ?>][<?php echo $field; ?>][value]\']').autocomplete({
	'source': function(request, response) {
		xhr = $.ajax({dataType:'json', url:'index.php?route=batch_editor/data/autocompleteByTableField&token=' + token + '&table=<?php echo $table; ?>&field=<?php echo $field; ?>&language_id=' + getLanguageId() + '&keyword=' + encodeURIComponent(request),
			success: function(json) {
				response($.map(json, function(item) {
