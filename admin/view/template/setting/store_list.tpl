...
<div id="content">
  <div class="page-header">
    <div class="container-fluid">
      <div style="display: none;" id="user-msg" class="alert alert-info" role="alert"></div>
      <div class="pull-right">
        <button id="cache-btn" class="btn btn-warning">Очистить КЕШ + обновить расширения</button>
  ...
  $(document).ready(function(){
    $('#cache-btn').click(function(){
      $.ajax({
        type: 'post',
        data: {flushFlag : 1},
        url: '/admin/index.php?route=setting/store/flush&token=<?=$token?>',
        success: function(response){

          $('#cache-btn').attr('disabled', 'disabled');
          $('#user-msg').text(response).toggle();

          setTimeout(function() {
            $('#user-msg').css('display', 'none');
            $('#cache-btn').removeAttr('disabled');
          }, 2500);
          ...
          setTimeout(function() {
            $.ajax({
              type: 'get',
              url: '/admin/index.php?route=extension/modification/refresh&token=<?=$token?>',
              success: function(response){
                if (response.length > 1)
                  $('#user-msg').text('Модификации обновлены');
