...
class ModelCatalogOCFilter extends Model
{
    ...
    public function getOptionByCategoriesId($categories_id)
    {
        ...
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "ocfilter_option oo LEFT JOIN " . DB_PREFIX . "ocfilter_option_description ood ON (ood.option_id = oo.option_id) LEFT JOIN " . DB_PREFIX . "ocfilter_option_to_category oo2c ON (oo.option_id = oo2c.option_id) WHERE oo2c.category_id IN (" . implode(',',
                $data) . ") AND ood.language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY oo.sort_order");
    ...
    public function getOptionsByCategoryId($category_id)
    {
        ...
        if ($options_query->num_rows) {
            ...
            $values_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "ocfilter_option_value oov LEFT JOIN " . DB_PREFIX . "ocfilter_option_value_description oovd ON (oov.value_id = oovd.value_id) WHERE oov.option_id IN (" . implode(',',
                    $options_id) . ") AND oovd.language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY oov.sort_order, ABS(oovd.name)");
    ...
    { # In options list
        ...
        if ($options_query->num_rows) {
            ...
            $values_query = $this->db->query("SELECT oov.value_id, oov.option_id, oovd.name FROM " . DB_PREFIX . "ocfilter_option_value oov LEFT JOIN " . DB_PREFIX . "ocfilter_option_value_description oovd ON (oov.value_id = oovd.value_id) WHERE oov.option_id IN (" . implode(',',
                    $options_id) . ") AND oovd.language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY oov.sort_order ASC, oovd.name DESC");
            ...
            $categories_query = $this->db->query("SELECT c.category_id, cd.name, oo2c.option_id FROM " . DB_PREFIX . "ocfilter_option_to_category oo2c LEFT JOIN " . DB_PREFIX . "category c ON (c.category_id = oo2c.category_id) LEFT JOIN " . DB_PREFIX . "category_description cd ON (cd.category_id = c.category_id) WHERE oo2c.option_id IN (" . implode(',',
                    $options_id) . ") AND cd.language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY c.sort_order, cd.name DESC");
    ...
    { # For product copy
        ...
        foreach ($query->rows as $key => $result) {
            $product_filter_value_data[$result['option_id']]['values'][(string)$result['value_id']] = array_merge($result,
                array('selected' => true));
    ...
    {
        // временная таблица для запоминания статуса фильтров
        $sql = '
              CREATE TEMPORARY TABLE ' . DB_PREFIX . 'ocfilter_options_statuses
              AS (SELECT store_attribute_id, status FROM  ' . DB_PREFIX . 'ocfilter_option)             
              ';
        $this->db->query($sql);
        // временная таблица для запоминания присутствия фильтров в категориях
        $sql = '
              CREATE TEMPORARY TABLE ' . DB_PREFIX . 'ocfilter_option_to_category_tmp
              AS (
                SELECT 
                    ootc.option_id AS option_id,
                    oo.store_attribute_id AS store_attribute_id,
                    ootc.category_id AS category_id 
                FROM  
                    ' . DB_PREFIX . 'ocfilter_option_to_category ootc
                LEFT JOIN 
                    ' . DB_PREFIX . 'ocfilter_option oo
                    ON oo.option_id=ootc.option_id
                )             
              ';
        $this->db->query($sql);
        ...
        if (!empty($data['copy_option'])) {
            ...
            $this->db->query("INSERT IGNORE INTO " . DB_PREFIX . "ocfilter_option (option_id, `type`, `status`, sort_order, image) SELECT option_id, '" . $this->db->escape($data['copy_type']) . "' AS type, '1' AS `status`, sort_order, IF(`type` = 'image', 1, 0) FROM `" . DB_PREFIX . "option`");
        ...
        if (!empty($data['copy_filter'])) {
            $this->db->query("INSERT IGNORE INTO " . DB_PREFIX . "ocfilter_option (option_id, `type`, `status`, sort_order) SELECT (filter_group_id + '" . (int)$last_option_id . "'), '" . $this->db->escape($data['copy_type']) . "' AS type, '1' AS `status`, sort_order FROM `" . DB_PREFIX . "filter_group`");
        ...
        if (!empty($data['copy_attribute'])) {
            ...
            $this->db->query("INSERT IGNORE INTO " . DB_PREFIX . "ocfilter_option (option_id, status, type, sort_order, store_attribute_id) SELECT (attribute_id + '" . (int)$last_option_id . "'), '1' AS status, '" . $this->db->escape($data['copy_type']) . "' AS type, sort_order, attribute_id AS store_attribute_id FROM " . DB_PREFIX . "attribute");
        ...
        if (!empty($data['copy_option']) || !empty($data['copy_filter']) || !empty($data['copy_attribute'])) {
            ...
            /* $this->db->query("
                   INSERT IGNORE INTO
                       " . DB_PREFIX . "ocfilter_option_to_category (option_id, category_id)
                   SELECT
                       oov2p.option_id, p2c.category_id
                   FROM
                       " . DB_PREFIX . "ocfilter_option_value_to_product oov2p
                   LEFT JOIN
                       " . DB_PREFIX . "product_to_category p2c
                       ON (p2c.product_id = oov2p.product_id)
                   WHERE
                       p2c.category_id != '0'
                   GROUP BY
                       oov2p.option_id, p2c.category_id
                       ");*/
            // вставляем значения из временной таблицы + те аттрибуты , которые актуальны для магазина
            $sql = "
            INSERT IGNORE INTO 
                " . DB_PREFIX . "ocfilter_option_to_category (option_id, category_id) 
            SELECT 
                tmp.option_id, tmp.category_id 
            FROM 
                " . DB_PREFIX . "ocfilter_option_to_category_tmp tmp
            WHERE 
                tmp.store_attribute_id IN 
                    (SELECT 
                        attribute_id 
                    FROM 
        ...
        // Set URL Aliases and update status from tmp table
        $query = $this->db->query("
        SELECT oo.option_id, ood.name, oos.status 
        FROM " . DB_PREFIX . "ocfilter_option oo 
        LEFT JOIN " . DB_PREFIX . "ocfilter_option_description ood 
            ON(oo.option_id = ood.option_id)
        LEFT JOIN " . DB_PREFIX . "ocfilter_options_statuses oos
            ON oos.store_attribute_id=oo.store_attribute_id
        WHERE ood.language_id = '" . (int)$this->config->get('config_language_id') . "' 
            AND oo.`keyword` = ''
            ");

        foreach ($query->rows as $row) {
            $this->db->query("
    	    UPDATE " . DB_PREFIX . "ocfilter_option 
    	    SET 
    	        `keyword` = '" . $this->db->escape($this->translit($row['name'])) . "', 
        ...
        // если есть одинаковые значения keyword в url_alias и ocfilter_option_value
        // фильтр неправильно определяет выбранную категорию
        /*if ($this->_hasOOVclones())
            $this->_deleteClonesFromOOV();*/

        $this->cache->delete('ocfilter');
        $this->cache->delete('product');
    }

    private function _hasOOVclones()
    {
        $sql = "
              SELECT 
                  COUNT(oov.value_id) AS clones 
              FROM 
                  " . DB_PREFIX . "ocfilter_option_value oov
              JOIN 
                  " . DB_PREFIX . "`url_alias` ua ON oov.keyword = ua.keyword
	          WHERE 
	              oov.`keyword` != ''
      ";
        ...
        return $query->row['clones'] > 0;
    }

    private function _deleteClonesFromOOV()
    {
        $sql = '
              UPDATE 
                  ' . DB_PREFIX . 'ocfilter_option_value oov
              JOIN 
                  ' . DB_PREFIX . '`url_alias` ua ON oov.keyword = ua.keyword
              SET 
                  oov.`keyword` = CONCAT(oov.`keyword`, "-") 
	          WHERE 
	              oov.`keyword` != ""
      ';
    ...
    {
        $this->db->query("INSERT INTO " . DB_PREFIX . "ocfilter_page SET title = '" . $this->db->escape($data['title']) . "', keyword = '" . $this->db->escape($data['keyword']) . "', description = '" . $this->db->escape($data['description']) . "', ocfilter_params = '" . $this->db->escape(trim($data['ocfilter_params'],
                '/')) . "', meta_title = '" . $this->db->escape($data['meta_title']) . "', meta_description = '" . $this->db->escape($data['meta_description']) . "', meta_keyword = '" . $this->db->escape($data['meta_keyword']) . "', category_id = '" . (int)$data['category_id'] . "', status = '" . (int)$data['status'] . "'");
    ...
    {
        $this->db->query("UPDATE " . DB_PREFIX . "ocfilter_page SET title = '" . $this->db->escape($data['title']) . "', keyword = '" . $this->db->escape($data['keyword']) . "', description = '" . $this->db->escape($data['description']) . "', ocfilter_params = '" . $this->db->escape(trim($data['ocfilter_params'],
                '/')) . "', meta_title = '" . $this->db->escape($data['meta_title']) . "', meta_description = '" . $this->db->escape($data['meta_description']) . "', meta_keyword = '" . $this->db->escape($data['meta_keyword']) . "', category_id = '" . (int)$data['category_id'] . "', status = '" . (int)$data['status'] . "' WHERE ocfilter_page_id = '" . (int)$ocfilter_page_id . "'");
