...
class ModelCatalogOCFilter extends Model {
    ...
    const SUB_CAT = 'subCat';

    public function getSubCategoriesUrlsByCategoryId($category_id)
    {
        $cacheKey = 'ocfilter.option.subcategories.' . $category_id . '.' . $this->config->get('config_language_id');
        $data = $this->cache->get($cacheKey);

        if ($data !== false)
            return $data;

        $sql = 'SELECT cat.category_id AS child_id, d.name AS `name`, cat.sort_order AS sort
                FROM ' . DB_PREFIX . 'category cat
                LEFT JOIN ' . DB_PREFIX . 'category_description d ON d.category_id=cat.category_id
                WHERE cat.status=1 AND cat.parent_id=' . (int)$category_id . ' AND cat.category_id IN 
                    (SELECT DISTINCT category_id FROM ' . DB_PREFIX . 'product_to_category)
                ';
        $query = $this->db->query($sql);

        if (empty($query->rows)) return $query->rows;

        $childSQL = 'SELECT `keyword`, `query` AS `cat_id` FROM ' . DB_PREFIX . 'url_alias WHERE ';
        $childParts = $childNames = $sort = array();
        foreach ($query->rows as $child) {
            $childParts[] = 'query="category_id=' . (int)$child['child_id'] . '"';
            $childNames[$child['child_id']] = $child['name'];
            $sort[$child['child_id']] = $child['sort'];
        }
        $childSQL .= implode(' OR ', $childParts);

        $childs = $this->db->query($childSQL);

        foreach ($childs->rows as $k => &$row) {
            if ($row['keyword'] == '') {
                unset($childs->rows[$k]);
                continue;
            }
            $row['cat_id'] = preg_replace('@category_id=@', '', $row['cat_id']);
            $row['name'] = $childNames[$row['cat_id']];
            $row['sort'] = $sort[$row['cat_id']];
        }

        uasort($childs->rows, function($a, $b){
            $a = $a['sort'];
            $b = $b['sort'];
        ...
        $this->cache->set($cacheKey, $childs->rows);
    ...
    public function getOCFilterOptionsByCategoryId($category_id)
    {
        ...
        if ($options_query->num_rows) {
            ...
            $values_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "ocfilter_option_value oov LEFT JOIN " . DB_PREFIX . "ocfilter_option_value_description oovd ON (oov.value_id = oovd.value_id) WHERE oov.option_id IN (" . implode(',',
                    $options_id) . ") AND oovd.language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY (oovd.name = '-') DESC, (oovd.name = '0') DESC, (oovd.name + 0 > 0) DESC, (oovd.name + 0), LENGTH(oovd.name), oovd.name");
        ...
        $this->cache->set('ocfilter.option.' . $category_id . '.' . $this->config->get('config_language_id'),
            $options_data);
  ...
  public function getProductSQL($params = array()) {
    ...
    $union[] = $this->getSubCategoryCounterSQl($data);
  ...
  private function getOptionValuesCounterSQL($data) {
    ...
    private function getSubCategoryCounterSQl($data)
    {
        $sql = "SELECT cat.category_id AS value_id, '" . self::SUB_CAT . "' AS option_id, COUNT(cat.category_id) AS total 
                FROM " . DB_PREFIX . "product p 
                LEFT JOIN " . DB_PREFIX . "product_to_category p2c ON (p.product_id = p2c.product_id)
                LEFT JOIN " . DB_PREFIX . "category cat ON cat.category_id=p2c.category_id
                WHERE p.status = '1' AND cat.parent_id = '" . (int)$data['filter_category_id'] . "'";
