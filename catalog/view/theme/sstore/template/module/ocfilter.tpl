<?php if ($options || $show_price) { ?>
    ...
    <div class="visible-xs" id="ocfilter-mobile">
        <nav class="navbar navbar-default">
            <div class="navbar-header">
                ...
                <button data-target="#navbar-ocfilter" data-toggle="collapse" class="btn btn-navbar navbar-toggle" type="button">
    ...
    <?php /*<div class="hidden" id="ocfilter-button">
    ...
    <div class="list-group" id="ocfilter-content">
                ...
                <?php foreach ($selecteds as $option) { ?>
                    <div class="ocfilter-option">
                        ...
                        <?php foreach ($option['values'] as $value) { ?>
                            <button type="button" onclick="location = '<?php echo $value['href']; ?>';" class="btn btn-xs btn-danger" style="padding: 1px 4px;">
                                <i class="fa fa-times"></i> <?php echo $value['name']; ?></button>
                        <?php } ?>
                    </div>
                <?php } ?>
                <?php $count = count($selecteds);
                $selected = $selecteds;
                $first = array_shift($selected); ?>
                <?php if ($count > 1 || count($first['values']) > 1) { ?>
                    <button type="button" onclick="location = '<?php echo $link; ?>';" class="btn btn-block btn-danger" style="border-radius: 0;">
                        <i class="fa fa-times-circle"></i> <?php echo $text_cancel_all; ?></button>
                <?php } ?>
            </div>
        <?php } ?>
        ...
        <?php if ($show_price) { # Price filtering ?>
            <div class="list-group-item ocfilter-option" data-toggle="popover-price">
                <div class="option-name">
                    ...
                    <?php /*<span id="price-from"><?php echo $min_price_get; ?></span>&nbsp;-&nbsp;<span id="price-to"><?php echo $max_price_get; ?></span><?php echo $symbol_right; ?>*/ ?>
                </div>

                <div class="popover-content">
                    <div class="form-inline">
                        <div class="form-group st">от</div>
                        <div class="form-group">
                            <input name="price[min]" value="" type="text" class="form-control input-sm" id="min-price-value">
                        </div>
                        <div class="form-group btw">до</div>
                        <div class="form-group">
                            <input name="price[max]" value="" type="text" class="form-control input-sm" id="max-price-value">
                        </div>
                        <div class="form-group ed">грн.</div>
                    </div>
                </div>
                ...
                <div id="ocfilter-button">
                    <button class="btn btn-primary" data-loading-text="<i class='fa fa-refresh fa-spin'></i> Загрузка..">Применить</button>
                </div>
            </div>
        ...
        <?php if ($show_options) { ?>
        <button type="button" class="btn btn-default btn-block" data-toggle="collapse" data-target="#ocfilter-hidden-options" aria-expanded="true" aria-controls="ocfilter-hidden-options">
            <i class="fa fa-angle-down"></i></button>
        <div class="collapse in" id="ocfilter-hidden-options" aria-expanded="true">
            <?php } else { ?>
            <button type="button" class="btn btn-default btn-block" data-toggle="collapse" data-target="#ocfilter-hidden-options" aria-expanded="false" aria-controls="ocfilter-hidden-options">
                <i class="fa fa-angle-down"></i></button>
            <div class="collapse" id="ocfilter-hidden-options" aria-expanded="false">
                <?php } ?>
                <?php } ?>

                <div class="list-group-item ocfilter-option<?= count($option['values']) > 5 ? ' expandable' : '' ?>" id="option-<?php echo $option['option_id']; ?>">
                    <div class="option-name">
                                    ...
                                    <?php } elseif ($option['type'] == 'radio' || $option['type'] == 'checkbox') { # Radio and Checkbox types start ?>
                                        ...
                                        <?php foreach ($option['values'] as $key => $value) { ?>
                                            ...
                                            <?php if ($show_values_limit && $show_values_limit == $key) { ?>
                                                <p>
                                                    <a href="#ocfilter-hidden-values-<?php echo $option['option_id']; ?>" data-toggle="collapse" aria-expanded="false" aria-controls="ocfilter-hidden-values"><?php echo $text_show_all; ?>
                                            ...
                                            <?php if ($value['selected']) { ?>
                                                <label id="v-<?php echo $value['id']; ?>" class="selected">
                                                    <input type="<?php echo $option['type']; ?>" id="value-<?= $i; ?>" name="ocfilter_filter[<?php echo $option['option_id']; ?>]" value="<?php echo $value['href']; ?>" checked="checked" class="ocf-target"/>
                                            ...
                                            <?php } elseif ($value['count']) { ?>
                                                <label id="v-<?php echo $value['id']; ?>">
                                                    <input type="<?php echo $option['type']; ?>" id="value-<?= $i; ?>" name="ocfilter_filter[<?php echo $option['option_id']; ?>]" value="<?php echo $value['href']; ?>" class="ocf-target"/>
                                                    <a href="<?php echo $value['href']; ?>"><?php echo $value['name']; ?></a>
                                                    <?php if ($show_counter) { ?>
                                            ...
                                            <?php } else { ?>
                                                <label id="v-<?php echo $value['id']; ?>" class="disabled">
                                                    <input type="<?php echo $option['type']; ?>" id="value-<?= $i; ?>" name="ocfilter_filter[<?php echo $option['option_id']; ?>]" value="" disabled="disabled" class="ocf-target"/>
    ...
    <script type="text/javascript">
        $(document).ready(function(){
            var min_price_filter = $('#scale-price').attr('data-start-min');
            var max_price_filter = $('#scale-price').attr('data-start-max');

            $('input#min-price-value').val(min_price_filter);
            $('input#max-price-value').val(max_price_filter);
        });
    </script>
<?php } ?>