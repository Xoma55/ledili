...
class ControllerCheckoutOctFastorder extends Controller { 
	public function index() {
        ...
        $this->document->addStyle('catalog/view/theme/sstore/stylesheet/additional.css');
		...
		$data['text_total_title'] = $this->language->get('text_total_title');
...
//            $subtraction = 0;
  	...
  	$data['products'] = array();
		...
		foreach ($products as $product) {
            ...
            if((int)$product_info['limitd']!=0 && $customerDiscount>(int)$product_info['limitd']) {
			    $product_discount=(int)$product_info['limitd'];
            } else {
                $product_discount=(int)$customerDiscount;
            }
			...
			$data['products'][] = array(
				...
				'total_disc'=> round($total-(((float)preg_replace('/[^\d.]/','',$total)*$product_discount)/100),2,PHP_ROUND_HALF_DOWN),
				'total'     => $total,
				'href'      => $this->url->link('product/product', 'product_id=' . $product['product_id'])
			);

			if($product_info['stock_status_id']=='6') $data['show_warning']=1;
		}
    ...
    $data['products_recurring'] = array();
		...
		foreach ($total_data as $total) {
		    ...
		    if($total['code']=='sub_total') continue;
...
//                // подключаем файл генерирования xml-файла и отправки его на почту
//                $this->load->model('sale/export');
//                // и вызываем его метод
//                $this->model_sale_export->createXMLOfCurrentOrder($this->session->data['order_id']);
//
//            }
